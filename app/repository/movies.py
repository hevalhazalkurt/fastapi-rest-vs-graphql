...
from app.schemas.movies import MovieOrder, MovieSort, MovieCreate, MovieUpdate
...
class MovieCRUD(AbstractCRUD):
    async def get_one(
        ...
        if extended:
            base_query = (
                ...
                .outerjoin(Genre, Genre.uuid == MovieGenreAssociation.genre_id).where(Movie.uuid == id)
    ...
    async def create(self, db: AsyncSession, movie_data: MovieCreate) -> Movie | None:
        ...
        if director:
            new_movie = Movie(title=movie_data.title, release_year=movie_data.release_year, uuid=uuid.uuid4(),
